...
export class BenchmarkService {
  ...
  analyzeCodeComplexity(code: string): CodeComplexityMetrics {
    ...
    const variableCount =
      (code.match(/local\s+\w+/g) || []).length +
      (code.match(/\w+\s*=/g) || []).length;

    const loopCount = (code.match(/\bfor\b|\bwhile\b|\brepeat\b/g) || [])
      .length;
  ...
  calculatePerformanceScore(
    ...
    const complexityScore = Math.max(
      0,
      1 - complexity.cyclomaticComplexity / 50,
    ); // 50 complexity = 0 score
  ...
  generateRecommendations(
    ...
    if (executionTime > 5) {
      recommendations.push(
        "Consider optimizing loops and recursive functions for better execution time",
      );
    }

    if (memoryUsage > 512 * 1024) {
      recommendations.push(
        "High memory usage detected - consider reducing variable scope and string operations",
      );
    }

    if (complexity.cyclomaticComplexity > 20) {
      recommendations.push(
        "High code complexity - consider breaking down large functions",
      );
    }

    if (complexity.functionCount < 3 && complexity.linesOfCode > 50) {
      recommendations.push(
        "Consider modularizing code into smaller functions for better maintainability",
      );
    }

    if (complexity.stringLiteralCount > 20) {
      recommendations.push(
        "Many string literals detected - string encoding may significantly impact performance",
      );
    }

    if (complexity.loopCount > 10) {
      recommendations.push(
        "Multiple loops detected - control flow obfuscation may add substantial overhead",
      );
    }
  ...
  async benchmarkScript(code: string): Promise<BenchmarkResult> {
    ...
    console.log(
      `Benchmark analysis completed in ${(endTime - startTime).toFixed(2)}ms`,
    );
  ...
  async compareBenchmarks(
    ...
    const executionTimeRatio =
      obfuscated.executionTime / original.executionTime;
    const memoryUsageRatio = obfuscated.memoryUsage / original.memoryUsage;
    const codeSizeRatio =
      obfuscated.codeComplexity.codeSize / original.codeComplexity.codeSize;
    const complexityIncrease =
      obfuscated.codeComplexity.cyclomaticComplexity -
      original.codeComplexity.cyclomaticComplexity;
    ...
    if (executionTimeRatio > 1.5) {
      recommendations.push(
        "Obfuscation significantly impacts execution time - consider lighter settings",
      );
    }

    if (memoryUsageRatio > 1.3) {
      recommendations.push(
        "Memory usage increased substantially - review string encoding settings",
      );
    }

    if (codeSizeRatio > 2.0) {
      recommendations.push(
        "Code size doubled - consider reducing obfuscation intensity",
      );
    }

    if (complexityIncrease > 15) {
      recommendations.push(
        "Code complexity increased significantly - may affect readability during debugging",
      );
    }

    if (performanceImpact === "minimal") {
      recommendations.push(
        "Excellent obfuscation efficiency with minimal performance impact",
      );
    }

    // Calculate optimization score (inverse of performance impact)
    const optimizationScore = Math.round(
      Math.max(0, 100 - (avgRatio - 1) * 100),
    );
